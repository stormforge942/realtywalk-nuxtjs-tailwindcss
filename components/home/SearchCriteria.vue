...
<template>
    <div class="absolute text-xl md:text-base z-30 top-0 left-0 md:top-[10px] md:left-4 flex w-screen md:w-max md:gap-x-2 bg-dark py-[1px] md:bg-transparent md:py-0">
        <button
        class="flex group w-full md:w-max"
        @click="onClickShowSearch(!homeStore.showSearch)">
            <div 
            :class="[homeStore.showSearch ? 'bg-secondary text-primary md:text-white md:bg-primary1' : 'text-white']"
            class="bg-primary1 md:bg-primary md:group-hover:bg-primary1 w-full h-[50px] md:w-32 md:h-10 flex items-center justify-center">
                {{ $t('home.map.btn_search_criteria') }}
            </div>
            <div class="bg-dark group-hover:bg-darkHover text-secondary min-w-[50px] h-[50px] md:min-w-10 md:h-10 flex items-center justify-center">
                <FontAwesome 
                :icon="faAngleDown"
        ...
        @click="onClickSaveSearch">
            <div 
            class="bg-primary1 md:bg-primary md:group-hover:bg-primary1 w-full h-[50px] text-white md:w-28 md:h-10 flex items-center justify-center">
                {{ $t('home.map.btn_save_search_short') }}
            </div>
            <div 
            class="bg-dark group-hover:bg-darkHover text-secondary min-w-[50px] h-[50px] md:min-w-28 md:h-10 flex md:hidden items-center justify-center">
                <FontAwesome 
                :icon="faBookmark"/>
            </div>
        </button>
        <div class="hidden md:flex gap-x-2 ml-7">
            <button 
        ...
        <div 
        :class="[homeStore.showResult && 'md:flex']"
