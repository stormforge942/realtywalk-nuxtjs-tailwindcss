...
<template>
    <div class="relative">
        <div class="absolute z-10 flex gap-x-2">
            <button
            class="flex group"
            @click="isOpen = !isOpen">
                <div 
                class="bg-primary group-hover:bg-primary1 text-white w-32 h-10 flex items-center justify-center">
                    {{ $t('home.map.btn_search_criteria') }}
                </div>
                <div class="bg-dark group-hover:bg-darkHover text-secondary w-10 h-10 flex items-center justify-center">
                    <FontAwesome 
                    :icon="faAngleDown"
            ...
            <button
            v-if="isOpen" 
            class="bg-primary hover:bg-primary1 w-28 h-10 flex items-center justify-center text-white">
                {{ $t('home.map.btn_save_search_short') }}
            </button>
        </div>
        <div
        :class="isOpen ? 'h-[calc(100vh-100px)] sm:h-[calc(100vh-120px)]' : 'h-0'" 
        class="bg-white shadow-md -left-4 -top-3 w-80 absolute duration-300 overflow-hidden">
            <div class="flex flex-col gap-4 mt-20 mx-4 max-h-[calc(100vh-320px)] overflow-y-scroll">
                <BaseSelect 
                :multiple="false"
            ...
            <div class="mx-4 flex flex-col gap-4 mt-4">
                ...
                <button type="button">{{ t('home.map.btn_clear_selected') }}</button>
            </div>
        </div>
    </div>
</template>